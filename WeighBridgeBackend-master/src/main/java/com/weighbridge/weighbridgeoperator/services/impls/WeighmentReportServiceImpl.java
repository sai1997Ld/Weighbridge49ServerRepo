...
public class WeighmentReportServiceImpl implements WeighmentReportService {
	...
	public WeighmentPrintResponse getAllWeighmentTransactions(Integer ticketNo, String userId) {
		/*
		 * HttpSession session = httpServletRequest.getSession(); String userId; String
		 * userCompany; String userSite; if (session != null &&
		 * session.getAttribute("userId") != null) { userId =
		 * session.getAttribute("userId").toString(); userSite =
		 * session.getAttribute("userSite").toString(); userCompany =
		 * session.getAttribute("userCompany").toString(); } else { throw new
		 * SessionExpiredException("Session Expired, Login again !"); }
		 */

		// Check transaction exists with ticketNo or not
		WeighmentTransaction weighmentTransaction = weighmentTransactionRepository
				.findByGateEntryTransactionTicketNo(ticketNo);
		...
		CompanyMaster companyMaster = companyMasterRepository
				.findById(weighmentTransaction.getGateEntryTransaction().getCompanyId())
				.orElseThrow(() -> new ResourceNotFoundException("Company is not found with id "
						+ weighmentTransaction.getGateEntryTransaction().getCompanyId()));
		weighmentPrintResponse.setCompanyName(companyMaster.getCompanyName());

		SiteMaster siteMaster = siteMasterRepository
				.findById(weighmentTransaction.getGateEntryTransaction().getSiteId())
				.orElseThrow(() -> new ResourceNotFoundException(
						"Site is not found with id " + weighmentTransaction.getGateEntryTransaction().getSiteId()));

		String companyAddress = siteMaster.getSiteName() + ", " + siteMaster.getSiteAddress();
		...
		String vehicleNo = vehicleMasterRepository
				.findVehicleNoById(weighmentTransaction.getGateEntryTransaction().getVehicleId());
		weighmentPrintResponse.setVehicleNo(vehicleNo);

		String materialName = materialMasterRepository
				.findMaterialNameByMaterialId(weighmentTransaction.getGateEntryTransaction().getMaterialId());
		weighmentPrintResponse.setMaterialName(materialName);

		String transporterName = transporterMasterRepository
				.findTransporterNameByTransporterId(weighmentTransaction.getGateEntryTransaction().getTransporterId());
		...
		if ("Inbound".equals(weighmentTransaction.getGateEntryTransaction().getTransactionType())) {
			String supplierName = supplierMasterRepository
					.findSupplierNameBySupplierId(weighmentTransaction.getGateEntryTransaction().getSupplierId());
		...
		if ("Outbound".equals(weighmentTransaction.getGateEntryTransaction().getTransactionType())) {
			String customerName = customerMasterRepository
					.findCustomerNameByCustomerId(weighmentTransaction.getGateEntryTransaction().getCustomerId());
		...
		// weighmentPrintResponse.setGrossWeight(weighmentTransaction.getGrossWeight());
		TransactionLog gwt = transactionLogRepository
				.findByTicketNoAndStatusCode(weighmentTransaction.getGateEntryTransaction().getTicketNo(), "GWT");
		if (gwt != null) {
		...
		// weighmentPrintResponse.setTareWeight(weighmentTransaction.getTareWeight());
		TransactionLog twt = transactionLogRepository
				.findByTicketNoAndStatusCode(weighmentTransaction.getGateEntryTransaction().getTicketNo(), "TWT");
		if (twt != null) {
	...
	private boolean isValidTransaction(GateEntryTransactionResponse response, LocalDate startDate, LocalDate endDate) {
		...
		// Check if the transaction is valid based on the status code and transaction
		// type
		...
		// Return true only if the transaction is within the date range and is valid
		// based on the status code
		return isWithinDateRange && isValidTransaction;
	}

	/**
	 * @param startDate The starting date for the report (format: YYYY-MM-DD),
	 *                  optional. If not provided, end Date will considered as
	 *                  startDate.
	 * @param endDate   The ending date for the report (format: YYYY-MM-DD),
	 *                  optional. If not provided, start Date will considered as
	 *                  endDate.
	 * @return
	 */
	public List<WeighbridgeReportResponse> generateWeighmentReport(LocalDate startDate, LocalDate endDate,
			String companyName, String siteName, String userId) {
		List<GateEntryTransactionResponse> gateEntryTransactionResponses = gateEntryTransactionService
				.getAllGateEntryTransactionForWeighmentReport(startDate, endDate, companyName, siteName, userId);
...
//        Map<String, Map<String, List<WeighbridgeReportResponseList>>> groupedReports = new HashMap<>();
		...
		gateEntryTransactionResponses.stream().forEach(response -> {
		...
		List<WeighbridgeReportResponse> reportList = groupedReports.entrySet().stream().map(entry -> {
			...
			double ch_SumQty = weighbridgeResponseList.stream()
					.filter(response -> response.getSupplyConsignmentWeight() != null)
					.mapToDouble(WeighbridgeReportResponseList::getSupplyConsignmentWeight).sum();

			double weight_SumQty = weighbridgeResponseList.stream()
					.filter(response -> response.getWeighQuantity() != null)
					.mapToDouble(WeighbridgeReportResponseList::getWeighQuantity).sum();

			double shtExcess_SumQty = weighbridgeResponseList.stream()
					.filter(response -> response.getExcessQty() != null)
					.mapToDouble(WeighbridgeReportResponseList::getExcessQty).sum();

			BigDecimal ch_SumQtyRounded = new BigDecimal(ch_SumQty).setScale(3, RoundingMode.HALF_UP);
			BigDecimal weight_SumQtyRounded = new BigDecimal(weight_SumQty).setScale(3, RoundingMode.HALF_UP);
			BigDecimal shtExcess_SumQtyRounded = new BigDecimal(shtExcess_SumQty).setScale(3, RoundingMode.HALF_UP);

			// Set the list and sums in the report
			report.setWeighbridgeResponse2List(weighbridgeResponseList);
			report.setCh_SumQty(ch_SumQtyRounded);
			report.setWeight_SumQty(weight_SumQtyRounded);
			report.setShtExcess_SumQty(shtExcess_SumQtyRounded);
	...
	/**
	 * simply it'll return the WeighbridgeReportResponseList to called method so
	 * that report can be generated Utility method to map
	 * GateEntryTransactionResponse to WeighbridgeReportResponseList
	 * 
	 * @param gateEntryResponse
	 * @return WeighbridgeReportResponseList or null if weighmentTransaction is not
	 *         found
	 */
	private WeighbridgeReportResponseList mapToWeighbridgeReportResponse(
		...
		if (challanDate != null) {
			...
			weighbridgeReportResponseList.setChallanDate(challanDate.format(dateFormatter));
		}
		...
		if (weighmentTransaction != null) {
			...
			Double netWeight = (Double) weighmentTransaction.getNetWeight() != null
					? weighmentTransaction.getNetWeight()
					: 0.0;
	...
	@Override
	public List<Map<String, Object>> generateCustomizedReport(List<String> selectedFields, LocalDate startDate,
			LocalDate endDate, String userId) {
		/*
		 * HttpSession session = httpServletRequest.getSession(); if (session == null ||
		 * session.getAttribute("userId") == null) { throw new
		...
		Predicate siteCompanyPredicate = criteriaBuilder.and(
				...
				criteriaBuilder.equal(gateEntryTransactionRoot.get("ticketNo"),
						weighmentTransactionRoot.get("gateEntryTransaction").get("ticketNo")));
		// Add date filtering predicates if startDate or endDate are provided
		if (startDate != null && endDate != null) {
			siteCompanyPredicate = criteriaBuilder.and(siteCompanyPredicate,
