...
public class CameraViewServiceImpl implements CameraViewService {
    ...
    @Override
	public String uploadImagesUserId(Integer ticketNo, MultipartFile frontImg1, MultipartFile backImg2,
			MultipartFile topImg3, MultipartFile bottomImg4, MultipartFile leftImg5, MultipartFile rightImg6,
			String role, String truckStatus, String userId) throws IOException {
		// TODO Auto-generated method stub
         ...
         UserMaster byId = userMasterRepository.findById(userId).orElseThrow(()->new RuntimeException("userId does not exist"));
         String companyName = byId.getCompany().getCompanyName();
         String siteName = byId.getSite().getSiteName();
         ...
         if (backImg2 != null && !backImg2.isEmpty()) {
             cameraView.setBackImg2(documentManagementService.saveDocumentString(backImg2, companyName, siteName, role));
         }
         if (topImg3 != null && !topImg3.isEmpty()) {
             cameraView.setTopImg3(documentManagementService.saveDocumentString(topImg3, companyName, siteName, role));
         }
         if (bottomImg4 != null && !bottomImg4.isEmpty()) {
             cameraView.setBottomImg4(documentManagementService.saveDocumentString(bottomImg4, companyName, siteName, role));
         }
         if (leftImg5 != null && !leftImg5.isEmpty()) {
             cameraView.setLeftImg5(documentManagementService.saveDocumentString(leftImg5, companyName, siteName, role));
         }
         if (rightImg6 != null && !rightImg6.isEmpty()) {
             cameraView.setRightImg6(documentManagementService.saveDocumentString(rightImg6, companyName, siteName, role));
         }
    ...
    public Map<String, byte[]> downloadImagesFromServerPath(Integer ticketNo, String role, String companyId, String siteId,String truckStatus) {
        if (role == null) {
            throw new ResourceNotFoundException("role is not given");
        }
        Integer roleId = roleMasterRepository.findRoleIdByRoleName(role);
        ...
        for (Map.Entry<String, String> entry : imageUrls.entrySet()) {
            ...
            if (remoteFilePath != null && !remoteFilePath.isEmpty()) {
                try {
                    // Extract only the file name from the URL
                    // Download the file using the adjusted path
                    byte[] fileBytes = documentManagementService.getFromServer(remoteFilePath);
                    imagesMap.put(imageType, fileBytes);
                } catch (IOException e) {
                    e.printStackTrace();
                    // Handle or log the exception as needed
